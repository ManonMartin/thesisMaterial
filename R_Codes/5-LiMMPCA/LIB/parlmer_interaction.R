#' Parallel mixed model with interaction
#'
...
#'
#' @return a list with the mkMerMod output for each response vector.
#'
...
parlmer_interaction <- function(design, outcomes, form, REML){
  ...
  # fmla1 <- as.formula(paste0(colnames(outcomes), form))
...
#### FIXME find fixNames (and randNames)
...
if ("CandiesJudges" %in% ranNames){

  Candies_mm <- model.matrix(~ Candies , contrasts.arg = list(Candies="contr.sum"))[,-1]

  Judges_mm <- matrix(0, ncol=nlevels(Judges), nrow=length(Judges),
                      dimnames = list(rownames(Candies_mm), paste0("Judges",levels(Judges))))
  for (i in 1:nlevels(Judges)){
    Judges_mm[Judges==levels(Judges)[i],i] <- 1
  }

  CandiesJudges_mm <- KhatriRao(t(Candies_mm), t(Judges_mm))
  rownames(CandiesJudges_mm) <- 1:nrow(CandiesJudges_mm)
  colnames(CandiesJudges_mm) <- rownames(design)
  # NEW ##################################
  # for (i in 1:length(reTrms_list)){
  #   reTrms_list[[i]]$Ztlist$`1 | CandiesJudges` <- CandiesJudges_mm
  # }
  ########################################
  ...
  for (i in 1:length(reTrms_list)){
    ...
    Ztlist <- reTrms_list[[i]]$Ztlist
    reTrms_list[[i]]$Zt <- do.call(rbind, Ztlist)

    if ("Judges" %in% ranNames){
      reTrms_list[[i]]$Lind <- c(rep(1, 44), rep(2, 11))
      # length(reTrms_list[[i]]$Lind)

      reTrms_list[[i]]$Gp <- as.integer(c(0,44,55))

      # as.matrix(reTrms_list[[i]]$Lambdat)
      # sum(as.matrix(reTrms_list[[i]]$Lambdat)) # ?? matrice diagonale
      # sum(diag(as.matrix(reTrms_list[[i]]$Lambdat)))
      # dim(as.matrix(reTrms_list[[i]]$Lambdat))
      #
      # class(reTrms_list[[i]]$Lambdat)

      reTrms_list[[i]]$Lambdat <- reTrms_list[[i]]$Lambdat[1:55,1:55]
    }else{
      reTrms_list[[i]]$Lind <- c(rep(1, 44))
      # length(reTrms_list[[i]]$Lind)

      reTrms_list[[i]]$Gp <- as.integer(c(0,44))
      ...
      reTrms_list[[i]]$Lambdat <- reTrms_list[[i]]$Lambdat[1:44,1:44]
    }
...
# NEW ##################################
  ...
  if(testNEW==TRUE){
  i=1
  a=as.matrix(reTrms_list[[i]]$Zt)

  colnames(a)
  reTrms_list[[i]]$Ztlist$`1 | CandiesJudges` <- CandiesJudges_mm

  dim(as.matrix(CandiesJudges_mm))
  Ztlist <- reTrms_list[[i]]$Ztlist
  reTrms_list[[i]]$Zt <- do.call(rbind, Ztlist)
  ...
  # library('Matrix')
  # m2 <- Matrix(data = 0, nrow = 55, ncol = 55, sparse = TRUE)
  # diag(m2) <- 1
  ...
  reTrms <- reTrms_list[[i]]
  X_list <- lapply(lFormula_res_lapply, function(x) x[["X"]])
  X <- X_list[[i]]
  fr <- fr_list[[i]]

  res_mkLmerDevfun <- mkLmerDevfun(fr = fr,X = X,
         reTrms = reTrms, REML = REML)

  res_optimizeLmer <- optimizeLmer(res_mkLmerDevfun)

  rho <- environment(res_mkLmerDevfun)
  mkMerMod(rho=rho, opt = res_optimizeLmer,
           fr = fr, reTrms = reTrms)
}
# END NEW #######################################

    # Apply mkLmerDevfun for 1 response to see if ok...
##########################################################
